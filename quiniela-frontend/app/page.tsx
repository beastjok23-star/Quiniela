      ...
      <div className="max-w-6xl mx-auto">
        ...
        {view === "manage_games" && (
          <div className="bg-white rounded-3xl p-8 shadow-sm">
            ...
            <h2 className="text-2xl font-bold mb-6">Quiniela {selectedQ} - Partidos</h2>
            ...
            <form onSubmit={handleAddGame} className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
              ...
              <button type="submit" disabled={isLoading} className="bg-green-800 text-white rounded-xl font-bold">{isLoading ? "..." : "Registrar"}</button>
            </form>
        ...
        {view === "puntuacion" && (
          <div className="bg-white rounded-3xl p-8 shadow-sm">
            ...
            <h2 className="text-2xl font-bold mb-6">Selecciona Quiniela a calificar</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[1, 2, 3, 4].map((num) => (
                <button key={num} onClick={() => { setSelectedQ(num); loadGames(num, "manage_scores"); }} className="py-8 bg-yellow-50 rounded-2xl font-bold text-xl">Quiniela {num}</button>
              ))}
            </div>
          </div>
        )}
        ...
        {view === "manage_scores" && (
          <div className="bg-white rounded-3xl p-8 shadow-sm">
            ...
            <h2 className="text-2xl font-bold mb-6">Asignar Ganadores - Quiniela {selectedQ}</h2>
            <div className="space-y-4">
              {games.map((game) => (
                <div key={game.id} className="p-4 border rounded-xl flex justify-between items-center">
                  <span>{game.team_a} vs {game.team_b}</span>
                  <div className="flex gap-2">
                    <button onClick={() => handleSetWinner(game.id, game.team_a)} className="bg-gray-100 p-2 rounded">Local</button>
                    <button onClick={() => handleSetWinner(game.id, 'Empate')} className="bg-gray-100 p-2 rounded">Empate</button>
                    <button onClick={() => handleSetWinner(game.id, game.team_b)} className="bg-gray-100 p-2 rounded">Visita</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {view === "ver_global" && (
          <div className="bg-white rounded-3xl p-8 shadow-sm">
            <button onClick={() => setView("main")} className="mb-6 p-2"><ArrowLeft /></button>
            <h2 className="text-2xl font-bold mb-6">Selecciona Quiniela para Ranking</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[1, 2, 3, 4].map((num) => (
                <button key={num} onClick={() => loadGlobalScores(num)} className="py-8 bg-blue-50 rounded-2xl font-bold text-xl">Quiniela {num}</button>
              ))}
            </div>
          </div>
        )}

        {/* --- PANTALLA 7: CLASIFICACIÓN --- */}
        {view === "view_global_scores" && (
          <div className="bg-white rounded-3xl p-8 shadow-sm">
            <button onClick={() => setView("ver_global")} className="mb-6 p-2"><ArrowLeft /></button>
            <h2 className="text-2xl font-bold mb-6">Clasificación - Quiniela {selectedQ}</h2>
